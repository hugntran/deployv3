...
const ChargingStationList: React.FC<ChargingStationListProps> = ({ locations }) => {
  return (
    <div className="overflow-x-auto">
      {locations.length === 0 ? (
        <div className="text-center text-gray-500 py-4">No charging stations available</div>
      ) : (
        <table className="min-w-full bg-white border border-gray-200 shadow-sm rounded-xl overflow-hidden">
          <thead>
            <tr className="bg-gray-100 text-gray-700 text-left">
              ...
              <th className="px-4 py-3 border-b">Charging gates</th>
                ...
                <tr key={location.id} className={index % 2 === 0 ? "bg-white" : "bg-gray-50 hover:bg-gray-100"}>
                  ...
                  <td className="px-4 py-2 border-b">
                    <div className="flex flex-wrap gap-2">
                      {Object.entries(location.chargingGateDistribution).map(([key, value]) => (
