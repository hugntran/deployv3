        ...
        <ul className="flex flex-col h-auto overflow-y-auto custom-scrollbar">
          {visibleNotifications.length === 0 ? (
            ...
            visibleNotifications.map((item) => (
              <li key={item.id}>
                <DropdownItem onItemClick={closeDropdown} className="flex gap-3 rounded-lg border-b border-gray-100 p-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5">
                  <span className="block">
                    <span className="mb-1.5 block text-theme-sm text-gray-900 dark:text-white">{item.title}</span>
                    <span className="text-theme-xs text-gray-600 dark:text-gray-400">{item.content}</span>
                  </span>
                </DropdownItem>
              </li>
            ))
        ...
        {!showAll && notifications.length > 5 && (
          <div className="pt-2 text-center">
            <button
              ...
              className="cursor-pointer rounded-md bg-gray-200 px-3 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600"
            >
              Show more
            </button>
          </div>
